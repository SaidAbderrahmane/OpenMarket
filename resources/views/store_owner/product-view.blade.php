...
<section class="py-5 bg-light">
    <div class="container">
        <div class="row px-4 px-lg-5 py-lg-4 justify-content-between">
            <div class="col-sm-4">
                <h1 class="h2 text-uppercase mb-0">Edit product</h1>
            </div>
...
<section class="py-5">
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input id="title" type="text" class="form-control" name="title" value="{{ $product->title}}" readonly>
        </div>
    </div>
    <div class="form-group row">
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input id="subtitle" type="text" class="form-control" name="subtitle" value="{{ $product->subtitle }}" readonly>
        </div>
    </div>
    <div class="form-group row">
        <label for="description" class="col-md-4 col-form-label text-md-right">Description</label>
        <div class="col-md-6">
            <textarea id="description" class="form-control" name="description" rows="15" readonly>
            {{ $product->description }}
            </textarea>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input id="stock" type="number" class="form-control" name="stock" value="{{ $product->stock }}" min="0" readonly>
        </div>
    </div>
    <div class="form-group row">
        <label for="store" class="col-md-4 col-form-label text-md-right">Store</label>
        <div class="col-md-6">
            <select name="store" class="form-control" id="store" readonly>
                @foreach ($stores as $store)
                <option value="{{ $store->id }}" {{ $product->store->id == $store->id ? 'selected' : '' }}>{{ $store->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <select name="categories[]" class="select form-control" id="categories" readonly multiple size=3>
                @foreach (App\Models\Category::all() as $category)
                <option value="{{ $category->id }}" @foreach ( $product->categories as $cat)
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <div style="padding: 10px;">
                <img id="image_display" src="{{ asset('storage/'.$product->image) }}" height="200" alt="">
            </div>
        </div>
    </div>
</section>
